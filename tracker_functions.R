...
remove_colony <- function(GPS_data) {
  ...
  warning(
    "READ-FIRST: Use function after using the polygon_check function or make sure the df contains an In_poly boolean column! Still need to adapt function to work per cycle instead of per bird!"
  )
  dummy <- NULL
  for (bird in unique(GPS_data$individual.local.identifier)) {
    ...
    t <-
      (as.data.frame(t(
        tapply(seq_along(ind_bird$In_poly), ind_bird$In_poly, max)
      )))
    t <- t$`TRUE`
    if (!is.null(t)) {
...
dbcluster_label <- function(GPS_data, eps, amount) {
  ...
  clustered_roosts$cluster_ID <-
    paste0(
      clustered_roosts$individual.local.identifier,
      "_cluster_",
      clustered_roosts$cluster
    )
  ...
  for (bird in unique(GPS_data$individual.local.identifier)) {
    ind_bird <-
      GPS_data  %>% filter(individual.local.identifier == bird)
    ind_bird <- ind_bird %>%
      mutate(
        t = lag(cluster, default = TRUE) != cluster & cluster == 0,
        c_p = cumsum(t),
...
subsample <- function(movebank_data, minutes) {
  ...
  movebank_data <- movebank_data %>%
    ...
    mutate(diff_sec = strptime(timestamp, "%Y-%m-%d %H:%M:%S") - lag(
  ...
  movebank_data <-
    setDT(movebank_data)[, .SD[.N], by = MESS::cumsumbinning(delta2, minutes - 1, cutwhenpassed = TRUE)]
...
append_rasterlayer <- function(lon, lat, tif_file) {
  ...
  # extract values
  raster_values   <-
    raster::extract(tif_file, SpatialPoints((coords),
...
minimal_distance <- function(lon1, lat1, lon2, lat2) {
  ...
  m1 <-
    apply(coords_A, 1, function(i)
      apply(coords_B, 1, function(j)
        distHaversine(i, j)))
