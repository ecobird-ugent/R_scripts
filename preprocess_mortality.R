...
# (meta-)data
animals <- read_sheet("https://docs.google.com/spreadsheets/d/1NZnLRdTBtqpktkQ3bjbwUPja7BXfkYZPvGqJplTR-Fs/edit#gid=0")
...
animals <- separate(data = animals, col = track_session_remarks, into = c("wing_length", "head_length", "tarsus_length", "death_info"), sep = "\\|")
animals$wing_length <- sub(".*:", "", animals$wing_length)
animals$tarsus_length <- sub(".*:", "", animals$tarsus_length)
animals$head_length <- sub(".*:", "", animals$head_length)

# remove malfunctional tags
animals <- subset(animals, !grepl("malfunc", animals$death_info, fixed = TRUE))

# cause of death to proper format
animals <- animals %>%
  mutate(death_info_cleaned = case_when(grepl("no further", animals$death_info) ~ 'No information',
...
GPS_data <- subset(GPS_data, `individual-local-identifier` %in% animals$metal_ring)
...
GPS_death <- subset(GPS_death, `individual-local-identifier` %in% t2$Var)
...
GPS_death <- subset(GPS_death, `individual-local-identifier` %in% t$`individual-local-identifier`)
